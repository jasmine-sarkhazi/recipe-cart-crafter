...
const MealPlan = () => {
  ...
  // Handle incoming schedule request from recipe cards
  useEffect(() => {
  ...
  const addMeal = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("meal_plan" as any).insert({
  ...
  const deleteMeal = useMutation({
    mutationFn: async (id: string) => {
      const { error } = await supabase.from("meal_plan" as any).delete().eq("id", id);
      if (error) throw error;
    },
    ...
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto px-4 py-8">
        ...
        <div className="grid gap-4 md:grid-cols-7">
          {DAYS.map((day) => {
            const dayMeals = meals.filter((m: any) => m.day_of_week === day);
            return (
              <Card key={day} className="min-h-[200px]">
                <CardHeader className="pb-2 pt-3 px-3">
                  <h3 className="text-sm font-semibold text-center">{day}</h3>
                </CardHeader>
                <CardContent className="px-3 pb-3 space-y-2">
                  {MEAL_TYPES.map((mealType) => {
                    const meal = dayMeals.find((m: any) => m.meal_type === mealType);
                    return (
                      <div key={mealType}>
                        ...
                        {meal ? (
                          ...
                          <button
                            ...
                            className="w-full rounded-md border border-dashed border-muted-foreground/30 py-1 text-xs text-muted-foreground hover:border-primary hover:text-primary transition-colors"
                          >
        ...
        <Dialog open={addDialogOpen} onOpenChange={setAddDialogOpen}>
          <DialogContent className="max-w-sm">
            <DialogHeader>
              <DialogTitle className="font-serif">Add Meal — {selectedDay}</DialogTitle>
            </DialogHeader>
            <div className="space-y-4">
              ...
              <Button className="w-full" disabled={!selectedRecipeId} onClick={() => addMeal.mutate()}>
                Add to Plan
              </Button>
            </div>
          </DialogContent>
