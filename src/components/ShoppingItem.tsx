...
const ShoppingItem = ({ id, ingredientName, quantity, unit, store, isPurchased, stores, onUpdate, onDelete }: ShoppingItemProps) => {
  return (
    <div className={`flex items-center gap-3 rounded-lg border bg-card p-3 transition-opacity ${isPurchased ? "opacity-50" : ""}`}>
      <Checkbox
        checked={isPurchased}
        onCheckedChange={(checked) => onUpdate(id, { is_purchased: !!checked })}
      />
      <span className={`min-w-[120px] text-sm font-medium ${isPurchased ? "line-through" : ""}`}>
        {ingredientName}
      </span>
      <Input
        type="number"
